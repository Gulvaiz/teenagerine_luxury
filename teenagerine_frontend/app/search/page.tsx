...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ [key: string]: string }> }) {
    // Pass query params directly to backend (assuming backend handles 'keyword' or 'q')
    // Mapping 'q' from frontend to 'keyword' in backend if needed
    const resolvedSearchParams = await searchParams;
    const backendParams: any = { ...resolvedSearchParams };
    if (resolvedSearchParams.q) {
        backendParams.search = resolvedSearchParams.q;
    }
        ...
        <>
            ...
            <main className="container" style={{ padding: '40px 20px' }}>
                ...
                <h1 className="section-title" style={{ textAlign: 'left', marginBottom: '30px', fontSize: '2rem', fontWeight: 'bold' }}>
                    Results for "{resolvedSearchParams.q || 'All Products'}"
                </h1>
